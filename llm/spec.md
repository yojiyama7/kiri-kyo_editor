...
- 境界カーソルと文末ダミーはslotではないため、このgeometryの対象外。

T は左右 slot を持つ。

```js
{ on, left: {enabled, text}, right: {enabled, text} }
```
...
- 始点にできるのは、表示値が `a`, `ad`, `副詞的目的格` の slot。
...
空の標識、空の構造、空の境界、空の矢印は JSON に出さない方針。
...
基礎slotは通常tokenのsingle slot、`d`のleft/right、tokenまたはgroup Tのleft/right。group参照は子groupが含む基礎slot集合へ再帰展開する。
...
- `h` / `l`: 表示 slot を左右移動。同じV下線に所属する葉slotが移動方向にあれば、内容が空でもそのslotを優先する。
...
- 通常時とV選択中の `h` / `l` は同じ純粋な移動先計算を使う。同じ下線の直接memberを最初に探索し、次に再帰的な葉slotを内側から外側のgroup順に探索する。方向側に候補があれば空slotでも優先する。
...
- 親groupの直接membersは、その親が作る局所グリッド上で同じ表示高の兄弟slotとして扱い、方向側の内側葉slotより先に選ぶ。非T子groupへ横移動するときはmark内容の有無にかかわらず `single` 標識slotを選び、空でも同じ位置にカーソルを表示する。
- 他groupに包含されない最上位group同士は、文ごとの暗黙rootが持つ兄弟memberとして `h` / `l` の直接移動対象にする。
- 非T group参照へ横移動するときは、markが空でも `single` 標識slotを選ぶ。上下移動や下線クリックで下線を選ぶ場合との表示targetの違いを混同しない。
