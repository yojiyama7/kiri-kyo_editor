...
## 2026-08-18 誘導adとslotなし`<`矢印

- `ead -> 誘導ad`と、`b < r Enter`でslotなしの`<`から現在tokenへ矢印を確定するブラウザテストを追加。修正前は2 failed, 110 passed（追加済み111件中）だった。
- `e`は通常slotでは標識文法を開始し、疑似トークン選択中だけ従来どおり表面文字列編集を優先する。`ea`に続く`d`で`誘導ad`を確定する。
- 直前の「`<`も境界slotを持つ」規則を廃止。`boundarySlots`の生成・保存・読込は`[`だけに限定し、旧データに`<`用slotがあっても読み込まない。
- BORDERの現在境界で最後に現れる`<`をslotなし境界参照として矢印始点にする。`r`でBORDERを終了してプレビューを表示し、通常の移動・Enter確定へ接続する。
- 全ブラウザテスト結果: 112 passed。

## 2026-08-18 非連続下線からgapへの横移動

- `Why is she taking`で`is`と`taking`だけを同じ非連続groupにしたfixtureを追加。`taking`から`h`すると未所属の`she`ではなく`is`へ飛ぶ失敗を確認し、修正前は1 failed, 112 passedだった。
- 原因は空の隣接slotを通常候補から除外した後、同じgroupの方向側葉を区間を跨いで優先する旧規則だった。
